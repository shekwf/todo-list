...
<!-- partial -->
  ...
  <script>
    $(document).ready(function(){
      ...
      $('.isDone').click(function(e){//done task
      ...
      $('#addFolderBtn').click(function(e){//add folder
          var input = $('input#addFolderInput');
          $.ajax({
            url : "process/ajaxHandler.php",
            method : "post",
            data : {action: "addFolder",folderName: input.val()},//داده به صورت ایجکس است و 2 کلید دارد
            success : function(response){
              if(response == '1'){//تعداد سطر های اضاف شده
                $('<li> <a href="#"><i class="fa fa-folder"></i>'+input.val()+'</a></li>').appendTo('ul.folder-list');//اضافه کردن به انتهای لیست
              }else{
                alert(response);
              }
            }
      ...
      $('#taskNameInput').on('keypress',function(e) {//add task
          e.stopPropagation();
          if(e.which == 13) {//کد کلید اینتر
              $.ajax({
                url : "process/ajaxHandler.php",
                method : "post",
                ...
                success : function(response){
                  if(response == '1'){
                    location.reload();//صفه ریلود میشه
                  }else{
                    alert(response);
                  }
                }
      ...
      $('#taskNameInput').focus();//بعد از رفرش صفه اماده نوشتن تسک است
    });
